...
describe("check 2: polling interval floor", () => {
  ...
  before(() => {
    saved = env.pollingIntervalMs;
  });
  after(() => {
    env.pollingIntervalMs = saved;
  });
...
describe("check 3: margin adjustment bounds", () => {
  ...
  before(() => {
    saved = env.marginAdjustment;
  });
  after(() => {
    env.marginAdjustment = saved;
  });
...
describe("check 4: taker >= maker min consistency", () => {
  it("holds for every market type", () => {
    const marketTypes = Object.keys(
      TAKER_MARGINS,
    ) as (keyof typeof TAKER_MARGINS)[];
