...
it("strips punctuation", () => {
    assert.equal(normalizeBucketPart("Kansas City Chiefs!"), "kansas-city-chiefs");
});

it("collapses multiple spaces", () => {
    assert.equal(normalizeBucketPart("New  England  Patriots"), "new-england-patriots");
});
...
it("h2h outcome 1 includes team name and full scope", () => {
    ...
    assert.equal(key, "event:bills-vs-chiefs-2024-01-21:scope:full:side:buffalo-bills");
});

it("h2h outcome 2 uses the other team name", () => {
    const market = makeMarket({ marketType: "h2h" });
    const key = getCorrelationBucketKey(market, 2);
    assert.equal(key, "event:bills-vs-chiefs-2024-01-21:scope:full:side:kansas-city-chiefs");
});

it("spreads outcome 1 shares bucket with h2h outcome 1 (same side, same event)", () => {
    const h2h = makeMarket({ marketType: "h2h" });
    const spread = makeMarket({ marketType: "spreads", marketSlug: "bills-chiefs-spread" });
    assert.equal(
