...
export function validateEnv(): void {
  ...
  if (Math.abs(totalWeight - 1) > 0.001) {
    throw new Error(
      `BOOKMAKER_WEIGHTS sum to ${totalWeight}, must equal 1.0`
    );
  }
  ...
  if (KELLY_MULTIPLIER <= 0 || KELLY_MULTIPLIER > 1) {
    throw new Error(
      `KELLY_MULTIPLIER=${KELLY_MULTIPLIER} must be in (0, 1]`
    );
  }

  if (MAKER_EVAL_EV_DROP <= 0 || MAKER_EVAL_EV_DROP > 0.1) {                    
    throw new Error(`MAKER_EVAL_EV_DROP=${MAKER_EVAL_EV_DROP} outside (0, 0.1] safety range`);                                                                                         
  }  
}
